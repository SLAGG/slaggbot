...
namespace SLAGGBot
{
	public class DoctorOctagonapus
		: ParserPluginBase
	{
		...
		{
			var m = Regex.Match (message, @"~dococ\s*(?<length>\d+)?", RegexOptions.Compiled | RegexOptions.IgnoreCase);
			if (m.Success)
			{
				var dococ = new StringBuilder ("DOCTOR OCTAGONAPUS BL");
